                          ...
                          int *hierarchy_depth)
{
    ...
    if ((name == NULL) ||
        ...
        (local_name == NULL) || (local_name_size <= 0) ||
        (base_name == NULL) || (base_name_size <= 0) ||
        (instance_index == NULL) ||
...
int main(int argc, char **argv)
{
    ...
    if (argc != 2)
    {
        fprintf(stderr, "usage: %s graph.yaml\n", argv[0]);
        return 1;
    }

    rc = yg_read_file(argv[1], &graph);
    if (rc != YG_OK)
    {
        fprintf(stderr,
