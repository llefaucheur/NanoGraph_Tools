...
int yn_read_file(const char *filename, YN_NodeManifest *m)
{
    ...
    enum { C_NONE=0, C_DATA_TYPE, C_FRAME_LENGTH, C_SAMPLE_RATE, C_NB_CHANNELS, C_SAME_AS };
        ...
        if (section == S_INTERFACES)
        {
            ...
            else if ((current_port >= 0) && (interface_kind != 0))
            {
                ...
                else if ((ind == 6) && (fmt != NULL) && yn_key_value(text, &key, &value))
                {
                    ...
                    else if ((strcmp(key, "same_as") == 0) && (*value == '\0')) format_constraint = C_SAME_AS;
                    else
                    {
                        format_constraint = C_NONE;
                ...
                else if ((ind >= 8) && (fmt != NULL) && yn_key_value(text, &key, &value))
                {
                    ...
                    else if (format_constraint == C_SAME_AS)
                    {
                        if (strcmp(key,"interface") == 0)
                        {
                            yn_copy(fmt->same_as_interface, YN_MAX_VALUE, value);
                            fmt->present |= YN_FMT_HAS_SAME_AS_INTERFACE;
                        }
                        else if (strcmp(key,"index") == 0)
                        {
                            if (!yn_int(value, &fmt->same_as_index)) goto badnum;
                            fmt->present |= YN_FMT_HAS_SAME_AS_INDEX;
                        }
                    }
