    ...
    function augmentTypeDefaults(type, def) {
        if (def.category === "input-function" || def.category === "output-function") {
            ...
            if (manifest && manifest.parameters && manifest.parameters.length) {
                /*
                 * Keep the external parameter-file field even when a manifest defines
                 * structured parameters. It is the forward-compatible escape
                 * hatch for parameters that are not in the manifest yet, and
                 * keeping it in defaults makes the editor/localStorage preserve
                 * it like every other node property.
                 */
                ensureDefault(def, "paramFile", "", false);
                ensureDefault(def, "paramtxt", "", false);
