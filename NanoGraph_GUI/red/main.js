...
var RED = (function() {
    ...
    function save(force) {
        ...
        if (RED.nodes.hasIO()) {
            ...
            yml += "\narcs:\n";
            for (var arcNodeIndex=0; arcNodeIndex<nns.length; arcNodeIndex++) {
                var arcNode = nns[arcNodeIndex];

                if (arcNode.wires) {
                    for (var portIndex=0; portIndex<arcNode.wires.length; portIndex++) {
                        var wires = arcNode.wires[portIndex];
                        if (!wires) continue;
                        ...
                        for (var wireIndex=0; wireIndex<wires.length; wireIndex++) {
                            ...
                            var src = RED.nodes.node(arcNode.id);
                            var dst = RED.nodes.node(arc.target);
                            if (!src || !dst || !arc.target) continue;

                            var src_name = make_name(src);
                            var dst_name = make_name(dst);

                            if (src["kind"] == "IO") {
                                yml += "  - OPort" + String(portIndex) + " IO:   " + src_name + "\n";
                            } else {
                                yml += "  - OPort" + String(portIndex) + " node: " + src_name + "\n";
                            }
            ...
            for (var ctrlIndex=0; ctrlIndex<nns.length; ctrlIndex++) {
                var ctrl = nns[ctrlIndex];
                var ctrlNode = RED.nodes.node(ctrl.id);
                if (ctrlNode && ctrlNode.outputs == 0 && ctrlNode._def.inputs == 0) {
                    var ctrlName = make_name(ctrl);
                    if (ctrlName == "script") {
                        yml += ctrl.type + " ";
                    } else {
                        yml += "//" + ctrl.type + " ";
                    ...
                    for (var padType=ctrl.type.length; padType<24; padType++) yml += " ";
                    yml += make_name(ctrl) + "\n";
                    for (var padId=ctrl.id.length; padId<14; padId++) yml += " ";
                }
            }
